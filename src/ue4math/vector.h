...
struct FVector {
   public:
    ...
    FVector operator*(float Value) const { return FVector(X * Value, Y * Value, Z * Value); }

    FVector GetNormalizedVector() const { return operator*(1.0f / sqrtf(X * X + Y * Y + Z * Z)); }

    void Normalize() { *this = GetNormalizedVector(); }

    float Length() const { return sqrtf(X * X + Y * Y + Z * Z); }

    float Distance(const FVector& v) const { return (v - *this).Length(); }

    FVector operator^(const FVector& v) const { return CrossProduct(v); }

    float operator|(const FVector& v) const { return DotProduct(v); }

    FVector GetSignVector() const { return FVector(Select(X, 1.0, -1.0), Select(Y, 1.0, -1.0), Select(Z, 1.0, -1.0)); }

    bool IsNearlyZero(float Tolerance = KINDA_SMALL_NUMBER) const { return fabs(X) <= Tolerance && fabs(Y) <= Tolerance && fabs(Z) <= Tolerance; }

    FRotator GetDirectionRotator() const;
};

static FVector operator*(float Value, const FVector& v) { return v.operator*(Value); }
