...
IMGUI_API bool IsWindowHovered(
    ImGuiHoveredFlags flags =
        0);  // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See ImGuiHoveredFlags_ for
             ...
             // to your underlying app, you should not use this function! Use the 'io.WantCaptureMouse' boolean for
...
IMGUI_API ImVec2 GetWindowPos();   // get current window position in screen space (note: it is unlikely you need to use
                                   // this. Consider using current layout pos instead, GetCursorScreenPos())
IMGUI_API ImVec2 GetWindowSize();  // get current window size (note: it is unlikely you need to use this. Consider using
...
IMGUI_API void SetNextWindowSizeConstraints(
    ...
    void* custom_callback_data =
        NULL);  // set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both min and
                // max of same axis to preserve current size (which itself is a constraint). Use callback to apply
...
IMGUI_API ImVec2 GetWindowContentRegionMin();  // content boundaries min for the full window (roughly (0,0)-Scroll), in
                                               // window coordinates
IMGUI_API ImVec2
...
IMGUI_API void PushTabStop(bool tab_stop);  // == tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by
...
//    GetContentRegionMax(), GetWindowContentRegion*(), PushTextWrapPos()
// - GetCursorScreenPos() = GetCursorPos() + GetWindowPos(). GetWindowPos() is almost only ever useful to convert from
// window-local to absolute coordinates.
IMGUI_API ImVec2 GetCursorScreenPos();  // cursor position in absolute coordinates (prefer using this, also more useful
...
// - A tooltip window can contain items of any types. SetTooltip() is a shortcut for the 'if (BeginTooltip()) {
// Text(...); EndTooltip(); }' idiom.
...
GetDragDropPayload();  // peek directly into the current payload from anywhere. returns NULL when drag and drop is
...
// Overlapping mode
IMGUI_API void
SetNextItemAllowOverlap();  // allow next item to be overlapped by a subsequent item. Useful with invisible buttons,
                            // selectable, treenode covering an area where subsequent items may need to be added. Note
                            // that both Selectable() and TreeNode() have dedicated flags doing this.
...
// - before v1.87, we used ImGuiKey to carry native/user indices as defined by each backends. About use of those legacy
// ImGuiKey values:
...
IMGUI_API bool IsKeyChordPressed(
    ImGuiKeyChord key_chord);  // was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S' as
                               // a key-chord. This doesn't do any routing or focus check, please consider using
...
IMGUI_API const char* GetKeyName(
    ImGuiKey key);  // [DEBUG] returns English name of the key. Those names a provided for debugging purpose and are not
...
IMGUI_API void SetNextFrameWantCaptureKeyboard(
    bool want_capture_keyboard);  // Override io.WantCaptureKeyboard flag next frame (said flag is left for your
                                  ...
                                  // e.g. force capture keyboard when your widget is being hovered. This is equivalent
                                  // to setting "io.WantCaptureKeyboard = want_capture_keyboard"; after the next
                                  // NewFrame() call.
...
IMGUI_API bool IsAnyMouseDown();  // [WILL OBSOLETE] is any mouse button held? This was designed for backends, but
                                  // prefer having backend maintain a mask of held mouse buttons, because upcoming input
                                  // queue system will make this invalid.
IMGUI_API ImVec2
...
IMGUI_API void SetNextFrameWantCaptureMouse(
    bool
        want_capture_mouse);  // Override io.WantCaptureMouse flag next frame (said flag is left for your application to
                              // handle, typical when true it instucts your app to ignore inputs). This is equivalent to
                              // setting "io.WantCaptureMouse = want_capture_mouse;" after the next NewFrame() call.
...
// (Legacy: bot 0 must always correspond to ImGuiChildFlags_Border to be backward compatible with old API using 'bool
...
enum ImGuiChildFlags_ {
    ...
    ImGuiChildFlags_Border = 1 << 0,  // Show an outer border and enable WindowPadding. (Important: this is always == 1
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1
                                              << 14,  // (WIP) Nav: left direction may move to this TreeNode() from any
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_AngledHeader =
        1
        << 18,  // TableHeadersRow() will submit an angled header row for this column. Note this will add an extra row.
...
enum ImGuiHoveredFlags_ {
    ...
    ImGuiHoveredFlags_AllowWhenBlockedByActiveItem = 1 << 7,  // Return true even if an active item is blocking access
    ...
    ImGuiHoveredFlags_AllowWhenOverlappedByWindow =
        1
        << 9,  // IsItemHovered() only: Return true even if the position is obstructed or overlapped by another window.
...
struct ImGuiIO {
    ...
    // value of BeginXXX() This is inconsistent with other BeginXXX functions and create confusion for many users. We
    // expect to update the API eventually. In the meanwhile we provide tools to facilitate checking user-code behavior.
    ...
    // Option to deactivate io.AddFocusEvent(false) handling. May facilitate interactions with a debugger when focus
    // loss leads to clearing inputs data. Backends may have other side-effects on focus loss, so this will reduce
    // side-effects but not necessary remove all of them. Consider using e.g. Win32's IsDebuggerPresent() as an
    // additional filter (or see ImOsIsDebuggerPresent() in imgui_test_engine/imgui_te_utils.cpp for a Unix compatible
    // version).
    bool ConfigDebugIgnoreFocusLoss;  // = false          // Ignore io.AddFocusEvent(false), consequently not calling
                                      // io.ClearInputKeys() in input processing.
    ...
    ImWchar PlatformLocaleDecimalPoint;  // '.'              // [Experimental] Configure decimal point e.g. '.' or ','
    ...
    IMGUI_API void SetAppAcceptingEvents(
        bool accepting_events);  // Set master flag for accepting key/mouse/text events (default to true). Useful if you
                                 // have native dialog boxes that are interrupting your application loop/refresh, and
                                 // you want to disable events being queued while your app is frozen.
    IMGUI_API void ClearEventsQueue();  // Clear all incoming events.
    IMGUI_API void ClearInputKeys();    // Clear current keyboard/mouse/gamepad state + current frame text input buffer.
                                        // Equivalent to releasing all keys/buttons.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    IMGUI_API void ClearInputCharacters();  // [Obsolete] Clear the current frame text input buffer. Now included within
                                            // ClearInputKeys().
#endif
    ...
    float Framerate;  // Estimate of application framerate (rolling average over 60 frames, based on io.DeltaTime), in
                      // frame per second. Solely for convenience. Slow applications may not want to use a moving
...
#endif
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    void* ImeWindowHandle;  // = NULL   // [Obsoleted in 1.87] Set ImGuiViewport::PlatformHandleRaw instead. Set this to
    ...
    ImGuiKeyChord KeyMods;  // Key mods flags (any of ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super flags,
                            // same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags. DOES NOT CONTAINS
    ...
    bool MouseWheelRequestAxisSwap;  // On a non-Mac system, holding SHIFT requests WheelY to perform the equivalent of
                                     // a WheelX event. On a Mac system this is already enforced by the system.
...
struct ImGuiListClipper {
    ...
    // inline ImGuiListClipper(int items_count, float items_height = -1.0f) { memset(this, 0, sizeof(*this)); ItemsCount
...
struct ImDrawList {
    ...
    IMGUI_API void AddEllipse(const ImVec2& center, float radius_x, float radius_y, ImU32 col, float rot = 0.0f,
                              int num_segments = 0, float thickness = 1.0f);
    IMGUI_API void AddEllipseFilled(const ImVec2& center, float radius_x, float radius_y, ImU32 col, float rot = 0.0f,
                                    int num_segments = 0);
    ...
    IMGUI_API void PathEllipticalArcTo(const ImVec2& center, float radius_x, float radius_y, float rot, float a_min,
    ...
    // OBSOLETED in 1.80 (Jan 2021) inline  void  PathBezierCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2&
...
struct ImFontConfig {
    ...
    int OversampleH;   // 2        // Rasterize at higher quality for sub-pixel positioning. Note the difference between
                       // 2 and 3 is minimal. You can reduce this to 1 for large glyphs save memory. Read
    ...
    float RasterizerMultiply;  // 1.0f     // Linearly brighten (>1.0f) or darken (<1.0f) font output. Brightening small
                               // fonts may be a good workaround to make them more readable. This is a silly thing we
    ...
    float RasterizerDensity;   // 1.0f     // DPI scale for rasterization, not altering other font metrics: make it easy
                               ...
                               // increase this it is expected that you increase font scale accordingly, otherwise
...
struct ImFontAtlas {
    ...
    IMGUI_API ImFont* AddFontFromMemoryTTF(
        void* font_data, int font_data_size, float size_pixels, const ImFontConfig* font_cfg = NULL,
    ...
    IMGUI_API ImFont* AddFontFromMemoryCompressedTTF(
        const void* compressed_font_data, int compressed_font_data_size, float size_pixels,
...
// static inline bool BeginChild(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags
// window_flags){ return BeginChild(str_id, size_arg, border ? ImGuiChildFlags_Border : ImGuiChildFlags_None,
// window_flags); } // Unnecessary as true == ImGuiChildFlags_Border static inline bool BeginChild(ImGuiID id, const
// ImVec2& size_arg, bool border, ImGuiWindowFlags window_flags)        { return BeginChild(id, size_arg, border ?
// ImGuiChildFlags_Border : ImGuiChildFlags_None, window_flags);     } // Unnecessary as true == ImGuiChildFlags_Border
...
IMGUI_API bool ImageButton(
    ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1),
